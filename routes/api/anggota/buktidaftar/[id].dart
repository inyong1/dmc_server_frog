...
Future<Response> _onPost(RequestContext context, String id) async {
  ...
  final List<int>? cardBytes = await formData.files['buktidaftar']?.readAsBytes();
  var savedBuktiDaftarName = '';
  ...
  if (cardBytes != null) {
    final file = File('${dir.path}/buktidaftar.jpg');
    final IOSink ioSink = file.openWrite()..add(cardBytes);
    await ioSink.flush();
    await ioSink.close();
    savedBuktiDaftarName = file.uri.pathSegments.last;
  }

  final jo = BaseResponse(success: true, message: 'berhasil').toJson();
  final uri = context.request.uri;
  jo['data'] ="${uri.scheme}://${uri.host}:${uri.port}/images/$id/$savedBuktiDaftarName";
